function removeFromArray(arr, elt){
  for (var i = arr.length; i >= 0; i--) {
    if (arr[i] == elt) {
      arr.splice(i, 1);
    }
  }
}


function heuristic(a, b) {
  var d = dist(a.i,a.j,b.i,b.j);
  // var d = abs(a.i - b.i) + abs(a.j - b.j);
  return d;
}

let cols = 10;
let rows = 10;
let grid = new Array(cols);

let openSet = [];
let closedSet = [];
let start;
let end;

let w, h;

let path = [];


function Cell(i, j) {
  this.i = i;
  this.j = j;
  this.f = 0;
  this.g = 0;
  this.h = 0;
  this.neighbours = [];
  this.previous = undefined;
  this.wall=false;
  this.noPath = false;

  if(random(1) < 0.1){
    this.wall = false;
  }


  this.show = function(col) {
    fill(col);
    if(this.wall == true){
      fill(0);
      noStroke();
      rect(this.i * w, this.j * h, w-1, h-1); //squers
      // ellipse(this.i * w + w/2, this.j * h + h/2, w/2, h/2);
    }
    if(this.noPath == true){
      fill(255,0,0);
      noStroke();
      rect(this.i * w, this.j * h, w-1, h-1); //squers
    }
      ellipse(this.i * w + w/2, this.j * h + h/2, w/2, h/2);

  }

  this.addNeighbours = function (grid) {
    if (i < cols -1) {
      this.neighbours.push(grid[this.i+1][this.j]);
    }
    if (i > 0) {
      this.neighbours.push(grid[this.i-1][this.j]);
    }
    if (j < rows-1) {
      this.neighbours.push(grid[this.i][this.j+1]);
    }
    if (j > 0) {
      this.neighbours.push(grid[this.i][this.j-1]);
    }
    if(i > 0 && j > 0 && !grid[this.i][this.j-1].wall && !!grid[this.i-1][this.j].wall){
      this.neighbours.push(grid[this.i - 1][this.j-1]);
    }
    if(i > 0 && j < rows-1 && !grid[this.i][this.j+1].wall && !!grid[this.i-1][this.j].wall){
      this.neighbours.push(grid[this.i - 1][this.j+1]);
    }
    if(i < cols-1 && j > 0 && !grid[this.i][this.j-1].wall && !!grid[this.i+1][this.j].wall){
      this.neighbours.push(grid[this.i + 1][this.j-1]);
    }
    if(i < cols-1 && j < rows-1 && !grid[this.i][this.j+1].wall && !!grid[this.i+1][this.j].wall){
      this.neighbours.push(grid[this.i+1][this.j+1]);
    }

  }
}


function setup() {
  createCanvas(900,900);
  console.log("Started!");

  w = width/cols;
  h = height/rows;

  for (var i = 0; i < cols; i++) {
    grid[i] = new Array(rows);
  }

  for (var i = 0; i < cols; i++) {
    for (var j = 0; j < rows; j++) {
      grid[i][j] = new Cell(i, j);
    }
  }
  for (var i = 0; i < cols; i++) {
    for (var j = 0; j < rows; j++) {
      grid[i][j].addNeighbours(grid);
    }
  }

  start = grid[0][0];
  start.wall = false;
  end = grid[cols - 1][rows - 1];
  end.wall = false;

  openSet.push(start);

}


function draw() {
  if(openSet.length > 0){
    var lowestIndex = 0;
    for (var i = 0; i < openSet.length; i++) {
      if(openSet[i].f < openSet[lowestIndex].f){
        lowestIndex = i;
      }
    }

    current = openSet[lowestIndex];

    if(current === end){

      var temp = current;
      path.push(temp);
      while(temp.previous){
        path.push(temp.previous);
        temp = temp.previous;
      }

      noFill();
      stroke(0,0,255);
      strokeWeight(w/2)
      beginShape();
        for (var i = 0; i < path.length; i++) {
          vertex(path[i].i * w + w/2, path[i].j * h + h/2);
        }
      endShape();


      noLoop();
      console.log("DONE!");
      return;
    }

    removeFromArray(openSet, current);
    closedSet.push(current);

    for (var i = 0; i < current.neighbours.length; i++) {
      var neighbour = current.neighbours[i];
      if (!closedSet.includes(neighbour) && !neighbour.wall) {
        var tempG = current.g+1;

        var newPath = false;
        if(openSet.includes(neighbour)){
          if(tempG < neighbour.g){
            neighbour.g = tempG;
            newPath =true;
          }
        }else{
          neighbour.g = tempG;
          newPath = true;
          openSet.push(neighbour);
        }
        if(newPath){
          neighbour.h = heuristic(neighbour, end);
          neighbour.f = neighbour.g + neighbour.h;
          neighbour.previous = current;
        }
      }
    }


  }else {

    noPath(end);

    noLoop();
    console.log("No solution");

  }


    background(255);

  for (var i = 0; i < cols; i++) {
    for (var j = 0; j < rows; j++) {
      grid[i][j].show(color(255));
    }
  }


  for (var i = 0; i < openSet.length; i++) {
    openSet[i].show(color(0,255,0));
  }

  for (var i = 0; i < closedSet.length; i++) {
    closedSet[i].show(color(255, 164, 36));
  }

  //   path = [];
  //   var temp = current;
  //   path.push(temp);
  //   while(temp.previous){
  //     path.push(temp.previous);
  //     temp = temp.previous;
  //   }
  //
  // noFill();
  // stroke(0,0,255);
  // strokeWeight(w/2)
  // beginShape();
  //   for (var i = 0; i < path.length; i++) {
  //     vertex(path[i].i * w + w/2, path[i].j * h + h/2);
  //   }
  // endShape();

}

function noPath(end) {
  end.noPath = true;
  end.show(color(255));
}
